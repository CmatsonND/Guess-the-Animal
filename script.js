...
(() => {
  ...
  const CATEGORY_MAP = [
    ...
    { cat: "bird",      kws: ["eagle","owl","sparrow","finch","penguin","duck","goose","swan","hawk","parrot","hummingbird","kingfisher","woodpecker","warbler","tern","albatross","falcon","heron","ibis","kestrel","egret","crane","loon","bustard","jacana","grebe","vulture","potoo","cockatoo","cockatiel","rooster","chicken","turkey","kite","swallow","toucan","oriole","bunting","raven","crow","condor"] },
    { cat: "fish",      kws: ["shark","eel","cod","salmon","trout","bass","tuna","catfish","snapper","piranha","pollock","sturgeon","mackerel","grouper","anchovy","anchovies","herring","perch","flounder","sole","gar","bowfin","sardine","paddlefish","ray","skate","wrasse","sunfish","drum"] },
    ...
    { cat: "dinosaur",  kws: ["saurus","raptor","pterosaur","pteranodon","pterodactyl","tyrannosaurus","allosaurus","stegosaurus","spinosaurus","brachiosaurus","velociraptor","triceratops","ichthyosaurus","plesiosaur","mosasaurus","titanoboa","megatherium","mastodon","mammoth","glyptodon","saber-toothed"] },
  ];
  ...
  function inferAttributes(name) {
    ...
    switch (category) {
      ...
      case "reptile":
        hasScales = true; laysEggs = true; numberOfLegs = 4;
        canSwim = /croc|gator|turtle|tortoise/i.test(t);
      ...
      case "crustacean":
        ...
        aquatic = true;
        numberOfLegs = t.includes("woodlouse") ? 14 : numberOfLegs;
        break;
  ...
  /** ---------------------------------------
   * State (DOM filled on DOMContentLoaded)
   * --------------------------------------- */
  let el = {};
  let hidden = null;
  let hintsUsed = 0;
  ...
  async function loadAnimalsTxt() {
    try {
      const res = await fetch("animals.txt", { cache: "no-cache" });
  ...
  function interpretQuestion(q) {
    ...
    // Legs: words (e.g., "four legs")
    const wordToNum = {
      "zero":0,"one":1,"two":2,"three":3,"four":4,"five":5,
      "six":6,"seven":7,"eight":8,"nine":9,"ten":10
    };
    const wordLegMatch = t.match(/\b(zero|one|two|three|four|five|six|seven|eight|nine|ten)\b.*\blegs?\b/);
    if (wordLegMatch) {
      const n = wordToNum[wordLegMatch[1]];
      return yesNo(hidden.numberOfLegs === n, cap(`It ${hidden.numberOfLegs === n ? "has" : "does not have"} ${n} legs.`));
    }

    // Legs: explicit count request
